...
#     for edge in edges:
#             assert Node(edge[1]) in gr._dict[Node(edge[0])]
...
#         gr.add_edge(edge[0], edge[1])
#     for edge in edges:
#         assert (Node(edge[0]), Node(edge[1])) in gr.edges()
...
#         gr.add_node(node)
#     for node in add_node:
#         assert Node(node) in list(gr._dict.keys())
...
#     g = Graph(iterable)
#     g.add_edge(n2, n1)
#     assert Node(n2) in g.neighbors(n1)
...
@pytest.mark.parametrize('iterable, start, path_list', DEPTH_TR)
def test_depth_first_traversal(iterable, start, path_list):
    gr = Graph([iterable])
    gr.add_edge(1, 2)
    gr.add_edge(1, 3)
    gr.add_edge(3, 2)
